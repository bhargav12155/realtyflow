...
export class HeyGenService {
  ...
  /**
   * Multi-scene video generation — up to 50 scenes in one API call.
   * Each scene has its own avatar character, voice, and optional background.
   */
  async generateMultiSceneVideo(options: {
    scenes: Array<{
      avatarId: string;
    ...
    const { scenes, title, aspectRatio = "16:9", caption = false, callbackUrl } = options;
    if (!scenes.length) throw new Error("At least one scene is required");
    if (scenes.length > 50) throw new Error("Maximum 50 scenes per video");

    const video_inputs = scenes.map((scene) => {
      const character = scene.avatarType === "talking_photo"
        ? { type: "talking_photo" as const, talking_photo_id: scene.avatarId }
        : {
            type: "avatar" as const,
            avatar_id: scene.avatarId,
            avatar_style: "normal",
            ...(scene.gestureIntensity && scene.gestureIntensity > 0
              ? { gesture: { intensity: scene.gestureIntensity } }
              : {}),
          };

      let voice: any;
      if (scene.audioAssetId) {
        voice = { type: "audio", audio_asset_id: scene.audioAssetId };
      } else if (scene.audioUrl) {
        voice = { type: "audio", audio_url: scene.audioUrl };
      } else {
        const cleanedScript = sanitizeScriptForTTS(scene.script ?? "");
        voice = {
          type: "text",
          input_text: cleanedScript.substring(0, 1500),
          voice_id: scene.voiceId || "119caed25533477ba63822d5d1552d25",
          speed: scene.speed ?? 1.0,
        };
      }

      const sceneObj: any = { character, voice };
      if (scene.background) sceneObj.background = scene.background;
      return sceneObj;
    });
    ...
    const payload: any = {
      ...
      video_inputs,
      dimension: {
        width:  aspectRatio === "16:9" ? 1280 : aspectRatio === "9:16" ? 720 : 1080,
        height: aspectRatio === "16:9" ? 720  : aspectRatio === "9:16" ? 1280 : 1080,
      },
      ...(title ? { title } : {}),
      ...(callbackUrl ? { callback_url: callbackUrl } : {}),
    };

    console.log("🎬 HeyGen multi-scene payload:", JSON.stringify(payload, null, 2));
    const raw = await this.makeRequest("/video/generate", "POST", payload);
    return parseHeygenVideoGenerateResponse(raw);
  }
  ...
  async uploadAudio(audioBuffer: Buffer, contentType: string): Promise<string> {
