...
export function ContentCalendar() {
  ...
  const [localGeneratedPosts, setLocalGeneratedPosts] = useState<
    typeof initialScheduledContent
  >([]);
  ...
  const updatePostMutation = useMutation({
    mutationFn: async ({
      ...
      const allowedFields: Array<keyof ScheduledPost> = [
        "status",
        "content",
        "scheduledFor",
        "hashtags",
  ...
  const approvePostMutation = useMutation({
    mutationFn: async (id: string) => {
      const response = await apiRequest("PATCH", `/api/scheduled-posts/${id}`, {
        status: "approved",
      });
      return await response.json();
    },
    onSuccess: () => {
      ...
      toast({
        title: "Post Approved",
        description:
          "The post has been approved and will be published on schedule.",
      });
    },
  });
  ...
  const duplicatePostMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description:
          "A copy has been created and scheduled for 1 week later. You can edit the schedule in the calendar.",
      });
    },
    onError: (error) => {
      toast({
  ...
  const scheduledContent = useMemo(() => {
    const transformedPosts = apiScheduledPosts.map((post) => {
      ...
      const platformKey =
        post.platform.toLowerCase() as keyof typeof platformColors;
  ...
  const setScheduledContent = (
    updater:
      | typeof initialScheduledContent
      | ((
          prev: typeof initialScheduledContent
        ) => typeof initialScheduledContent)
  ) => {
    if (typeof updater === "function") {
      setLocalGeneratedPosts((prevLocal) => {
        const seedContent =
          apiScheduledPosts.length > 0 ? [] : initialScheduledContent;
        const transformedAPI = apiScheduledPosts.map((p) => {
          const platformKey =
            p.platform.toLowerCase() as keyof typeof platformColors;
          return {
            id: `api-${p.id}`,
            title: p.postType
              ? postTypeLabels[p.postType] || p.postType
              : "Social Post",
  ...
  const getContentForDate = (date: Date) => {
    ...
    return scheduledContent.filter(
      (content) => content.date.toDateString() === dateStr
    );
  };
  ...
  const getHolidayForDate = (date: Date) => {
    ...
    return holidayRecommendations[
      dateStr as keyof typeof holidayRecommendations
    ];
  };
    ...
    try {
      ...
      const response = await fetch("/api/ai/schedule-content", {
        ...
        body: JSON.stringify({
          ...
          focus: "high-impact",
          prompt:
            "You are a Luxury real estate agent. Create 2 weeks worth of social media posts. Optimize what days are best for each platform.",
        }),
      ...
      if (error.name === "AbortError") {
        toast({
          ...
          description:
            "AI generation took too long (60s limit). Please try again.",
          variant: "destructive",
        });
      } else {
        toast({
          ...
          description:
            "Failed to generate optimized schedule. Please try again.",
          variant: "destructive",
        });
      }
    } finally {
          ...
          <div className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-4 gap-4">
                <Select
                  value={`${format(selectedDate, "MMMM")} ${format(
                    selectedDate,
                    "yyyy"
                  )}`}
            ...
            <div className="mt-6">
              ...
              <div className="space-y-3">
                {scheduledContent.map((content) => (
                  ...
                  >
                    <div
                      className={`w-2 h-2 ${content.color} rounded-full`}
                    ></div>
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {content.type} • {format(content.date, "MMM d")}{" "}
                        {content.time}
                      </p>
                    </div>
                    <Button
            ...
            <div className="space-y-3">
              {isLoading ? (
                ...
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm">
                    Click "Generate 30-Day Plan" to get started
                  </p>
                </div>
              ) : (
                scheduledContent.map((content) => {
                    ...
                    >
                      <div className="flex items-start justify-between">
                          ...
                          <div className="text-sm text-muted-foreground mb-2">
                            <div className="flex items-center gap-3">
                              <span className="flex items-center gap-1">
                                ...
                                {format(content.date, "MMM d, yyyy")} at{" "}
                                {content.time}
                              </span>
                        ...
                        <div className="flex items-center gap-2 ml-4">
                          ...
                          {content.id.startsWith("api-") && (
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => {
                                const postId = content.id.replace("api-", "");
                                if (
                                  confirm(
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  {String(previewContent.id).startsWith("api-") && (
                    <Button
                      ...
                      size="sm"
                      onClick={() => {
                        const apiPostId = String(previewContent.id).replace(
                          "api-",
                          ""
                        );
                        const originalPost = apiScheduledPosts.find(
                          (p) => p.id === apiPostId
                        );
                        if (originalPost) {
