...
function ThumbCollage({ thumbs }: { thumbs: { id: string; thumbnailUrl: string | null }[] }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-1 w-[148px] h-[148px] flex-shrink-0">
      {slots.map((src, i) => (
        <div key={i} className="bg-neutral-300/70 rounded-md overflow-hidden">
          {src ? (
...
export function BoardCard({ board }: { board: BoardSummary }) {
  ...
  return (
    <Link href={`/boards/${board.id}`}>
      <a
        className={`block bg-gradient-to-br ${tint} rounded-2xl p-4 hover:ring-2 hover:ring-neutral-300 transition cursor-pointer`}
        data-testid={`card-board-${board.id}`}
      >
        <div className="text-[10px] font-semibold tracking-wider text-neutral-700 mb-0.5 uppercase">
          {first} {highlight && <span className="text-neutral-900">{highlight}</span>}
        </div>
        <div className="text-[10px] text-neutral-500 mb-3">{relativeTime(board.updatedAt)}</div>
...
export function NewBoardCard({ onClick }: { onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="bg-neutral-100/70 border border-dashed border-neutral-300 rounded-2xl p-4 flex items-center justify-center min-h-[220px] hover:bg-neutral-200/60 transition cursor-pointer"
      data-testid="card-new-board"
    >
      <div className="flex flex-col items-center gap-2">
        <Plus className="w-8 h-8 text-neutral-700" strokeWidth={1.5} />
        <div className="text-[10px] text-neutral-500">New board</div>
      </div>
    </button>
  );
}
