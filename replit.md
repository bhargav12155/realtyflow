...
- **Luma UNI-1 / UNI-1 Max Image Generation**: Separate Luma product (NOT Dream Machine) wired through `server/services/luma-agents.ts` against `https://agents.lumalabs.ai/v1/generations` and authenticated with `LUMA_AGENTS_API_KEY`. Exposed as image providers `uni-1-image` and `uni-1-max-image` in the Boards Build dispatcher (`server/routes/boards-chat.ts`), in `/api/images/generate` via `provider: "uni-1" | "uni-1-max"`, in the shared image picker model dropdown, and as an AI Assistant intent passthrough (saying "use uni-1" / "luma uni" routes the chat image generation through the new service). Supports text→image and image_edit (using one of the referenced board assets as `source` and the rest as `image_ref`), the 9 documented aspect ratios, and snaps manga style to portrait per the API contract. Generated images are downloaded via the SSRF-safe fetcher and re-hosted through `persistImageBufferPublic` so the presigned URLs don't expire on users. When `LUMA_AGENTS_API_KEY` is missing the dispatch silently falls back to gemini-image (Boards) / OpenAI (`/api/images/generate`) so the optional key is never a hard requirement. Existing `server/services/luma.ts` (Dream Machine video) is intentionally untouched.
