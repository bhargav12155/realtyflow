...
export function registerBoardsRoutes(
  ...
  // Compile all ready video assets from a board into a single downloadable MP4.
  // Winners (assets whose most recent evalHistory entry has outcome="winner")
  // are placed first; remaining ready videos follow in canvas position order.
  // Uses ffmpeg concat under the hood — the same pipeline used by property tours.
  app.post("/api/boards/:id/compile", auth, async (req: Request, res: Response) => {
    ...
    try {
      ...
      const isWinner = (a: (typeof videoAssets)[number]) => {
        const hist = Array.isArray(a.evalHistory) ? a.evalHistory : [];
      ...
      const tempDir = await fsModule.mkdtemp(path.join(os.tmpdir(), "board-compile-"));
      ...
      try {
        ...
        // Normalise each clip to the same codec/resolution so concat works
        const normDir = path.join(tempDir, "norm");
        await fsModule.mkdir(normDir, { recursive: true });
        const normalizedFiles: string[] = [];
        for (let i = 0; i < downloadedFiles.length; i++) {
          const normFile = path.join(normDir, `norm-${i}.mp4`);
          await execAsync(
            `ffmpeg -y -i "${downloadedFiles[i]}" -vf "scale=1280:720:force_original_aspect_ratio=decrease,pad=1280:720:(ow-iw)/2:(oh-ih)/2,fps=30,format=yuv420p" -c:v libx264 -preset fast -crf 22 -an -movflags +faststart "${normFile}"`,
            { timeout: 120000 },
          );
          normalizedFiles.push(normFile);
        }

        const outputFile = path.join(tempDir, "compiled.mp4");

        if (normalizedFiles.length === 1) {
          // Only one clip — just serve it directly
          await fsModule.copyFile(normalizedFiles[0], outputFile);
        } else {
          // Concat all normalised clips
          const concatList = normalizedFiles.map((f) => `file '${f}'`).join("\n");
          const concatPath = path.join(tempDir, "concat.txt");
          await fsModule.writeFile(concatPath, concatList);
          await execAsync(
            `ffmpeg -y -f concat -safe 0 -i "${concatPath}" -c copy -movflags +faststart "${outputFile}"`,
            { timeout: 300000 },
          );
        }

        const outBuf = await fsModule.readFile(outputFile);
        const boardTitle = board.title ?? "board";
        const safeTitle = boardTitle.replace(/[^a-z0-9_-]/gi, "-").toLowerCase();
      ...
      console.error("[boards/compile] error:", msg);
      res.status(500).json({ error: `Compile failed: ${msg}` });
    }
  });
