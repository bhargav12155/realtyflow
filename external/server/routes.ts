...
<noscript><a href="/#social">Click here to continue</a></noscript>
  ...
  app.post(
    ...
    createAiChatHandler({
      ...
      loadAnthropic: () => import("./services/anthropic"),
      loadGemini: () => import("./services/gemini"),
    }),
  );
...
Return ONLY valid JSON in this format: {"opportunities": [{...}, {...}, ...]}`;
  ...
  app.post("/api/scheduled-posts/upload-media", requireAuth, mediaUpload.single("media"), autoProcessImageMiddleware, async (req: any, res) => {
    try {
      if (!req.file) {
...
Return JSON with: { "content": "post text", "hashtags": ["hashtag1", "hashtag2"] }`;
  ...
  app.post(
    ...
    requireAuth,
    aiAssistantUpload.array('files', 5),
    autoProcessImageMiddleware,
    createAiAssistantChatHandler({
      multiOpenAI,
      db,
      aiAssistantMessagesTable: aiAssistantMessages,
      s3UploadService,
      loadAnthropic: () => import("./services/anthropic"),
      loadGemini: () => import("./services/gemini"),
    }),
