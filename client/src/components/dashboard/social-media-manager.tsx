...
import { Facebook, Instagram, Linkedin, Twitter as X, Image, Calendar, Sparkles, Home, Tag, CheckCircle, TrendingDown, Eye, Upload, ImageIcon, CreditCard, Brain, Music, Video } from "lucide-react";
...
const stockPhotos = [
  ...
  { id: 2, url: "https://images.unsplash.com/photo-1449844908441-8829872d2607?auto=format&fit=crop&w=400&q=80", title: "Real Estate Keys" },
  ...
  { id: 6, url: "https://images.unsplash.com/photo-1613490493576-7fde63acd811?auto=format&fit=crop&w=400&q=80", title: "Sold Sign" },
  { id: 7, url: "https://images.unsplash.com/photo-1582407947304-fd86f028f716?auto=format&fit=crop&w=400&q=80", title: "House with Garden" },
  { id: 8, url: "https://images.unsplash.com/photo-1570129477492-45c003edd2be?auto=format&fit=crop&w=400&q=80", title: "Neighborhood" },
];
...
export function SocialMediaManager() {
  ...
  useEffect(() => {
    if (selectedProperty && selectedProperty.photoUrls && selectedProperty.photoUrls.length > 0) {
    ...
    try {
      ...
      if (!youtubeAccount || !youtubeAccount.isConnected) {
        ...
        toast({
          ...
          description: "Redirecting to Google to connect your YouTube account...",
        });
        ...
        if (videoFile) {
          ...
          toast({
            ...
            description: "Please re-upload your video after YouTube authentication.",
            variant: "default",
          });
        }
      ...
      if (videoFile) {
        ...
        formData.append('accessToken', (youtubeAccount as any).accessToken || '');
  ...
  useEffect(() => {
    ...
    if (pendingPost) {
      ...
      toast({
        ...
        description: "Your content is ready to post. Click 'Post' to publish to YouTube.",
      });
    }
  }, [accounts]);
  ...
  const postMutation = useMutation({
    mutationFn: async (data: { content: string; platforms: string[] }) => {
      ...
      if (data.platforms.includes('youtube')) {
        return await handleYouTubePost(data.content, uploadedVideo || undefined);
      }
      
      // Handle other platform-specific posting
      if (data.platforms.includes('facebook')) {
        // Use Facebook Pages API for Facebook posting
        const facebookResponse = await apiRequest("POST", "/api/facebook/post", {
        ...
        const instagramResponse = await apiRequest("POST", "/api/instagram/post", {
          content: data.content
          // Instagram User ID will be read from environment variables
        });
        return instagramResponse.json();
      } else if (data.platforms.includes('x') || data.platforms.includes('twitter')) {
  ...
  const facebookPostMutation = useMutation({
    mutationFn: async (data: { content: string; pageId?: string; photo?: File }) => {
      const formData = new FormData();
    ...
    onSuccess: (data) => {
      toast({
        ...
        description: data.message || "Your content has been posted to Facebook successfully.",
      });
      setPostContent("");
  ...
  const optimizeContentMutation = useMutation({
    mutationFn: async (data: { topic: string; platform: string }) => {
      const response = await apiRequest("POST", "/api/content/social-post", data);
      return response.json();
    },
    onSuccess: (data) => {
      ...
      toast({
        ...
        description: "Generated platform-specific content for better engagement",
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const generatePropertyContent = (property: Property, postType: string, platform: string) => {
    ...
    const bedBathText = `${property.bedrooms ? `🛏️ ${property.bedrooms} bed` : ''} ${property.bathrooms ? `🛁 ${property.bathrooms} bath` : ''} ${property.squareFootage ? `📐 ${property.squareFootage.toLocaleString()} sqft` : ''}`;
    const neighborhoodTag = property.neighborhood ? property.neighborhood.replace(/\s+/g, '') : '';
...
${property.neighborhood ? `📍 Located in desirable ${property.neighborhood}` : ''}

Contact Mike Bjork at Berkshire Hathaway HomeServices for more information!

#JustListed #OmahaRealEstate #MikeBjork #BHHS ${neighborhoodTag ? `#${neighborhoodTag}` : ''}`,
...
✨ ${property.bedrooms}BD ${property.bathrooms}BA${property.squareFootage ? ` | ${property.squareFootage.toLocaleString()} sqft` : ''}

${property.description.substring(0, 150)}...

DM for details! 📩

#JustListed #OmahaHomes #RealEstate #MikeBjork ${neighborhoodTag ? `#${neighborhoodTag}` : ''}`,
        
        x: `🏠 JUST LISTED!\n\n${property.address}\n${formatPrice(property.listPrice)}\n${property.bedrooms}BD ${property.bathrooms}BA\n\n${property.description.substring(0, 100)}...\n\nContact Mike Bjork for details!\n\n#JustListed #OmahaRealEstate`,
        
        youtube: `🏠 NEW LISTING: ${property.address} | ${formatPrice(property.listPrice)}

Welcome to this stunning ${property.bedrooms} bedroom, ${property.bathrooms} bathroom home in ${property.neighborhood || property.city}! This beautiful ${property.squareFootage ? property.squareFootage.toLocaleString() + ' square foot ' : ''}${property.propertyType.toLowerCase()} offers everything you've been looking for.

${property.description}

${property.neighborhood ? `Located in the desirable ${property.neighborhood} neighborhood, ` : ''}this property is perfectly positioned for ${property.city} living. Whether you're a first-time homebuyer or looking to upgrade, this home offers incredible value at ${formatPrice(property.listPrice)}.

Key Features:
${property.features && Array.isArray(property.features) ? property.features.slice(0, 5).map(feature => `• ${feature}`).join('\n') : '• Beautifully maintained interior\n• Great neighborhood location\n• Move-in ready condition'}
...
#JustListed #OmahaRealEstate #${property.neighborhood ? property.neighborhood.replace(/\s+/g, '') : 'OmahaHomes'} #MikeBjork #BerkshireHathaway #RealEstate #HomeTour`
      },
...
${property.neighborhood ? `Properties in ${property.neighborhood} continue to perform well in our market.` : ''}
...
${property.neighborhood ? `${property.neighborhood} market staying strong! 💪` : ''}
...
I'm thrilled to share another successful sale in ${property.neighborhood || property.city}! This beautiful ${property.bedrooms} bedroom, ${property.bathrooms} bathroom home has found its perfect new owners.

${property.description.substring(0, 300)}

This ${property.squareFootage ? property.squareFootage.toLocaleString() + ' square foot ' : ''}property sold quickly, showcasing the continued strength of ${property.neighborhood ? `the ${property.neighborhood}` : 'our local'} real estate market.
...
${property.neighborhood ? `Properties in ${property.neighborhood} continue to perform exceptionally well, with strong buyer demand and competitive pricing.` : 'The Omaha market remains strong with excellent opportunities for both buyers and sellers.'}
...
#JustSold #OmahaRealEstate #${property.neighborhood ? property.neighborhood.replace(/\s+/g, '') : 'OmahaHomes'} #MikeBjork #BerkshireHathaway #RealEstateSuccess #SoldHomes`
      },
...
${property.neighborhood ? `Don't miss this opportunity in ${property.neighborhood}!` : "Don't miss this opportunity!"}
...
${property.neighborhood ? `Great opportunity in ${property.neighborhood}!` : 'Great opportunity!'}

DM me now! 📩

#PriceImprovement #OmahaHomes #Opportunity`,
        
        x: `💰 PRICE IMPROVED!\n\n${property.address}\nNOW ${formatPrice(property.listPrice)}!\n\n${property.bedrooms}BD ${property.bathrooms}BA\n\n${property.neighborhood ? `${property.neighborhood} opportunity!` : 'Great opportunity!'}\n\nMike Bjork | BHHS\n\n#PriceImprovement`,
        
        youtube: `💰 PRICE IMPROVEMENT! ${property.address} | Now ${formatPrice(property.listPrice)}

Exciting news! This beautiful ${property.bedrooms} bedroom, ${property.bathrooms} bathroom home just had a strategic price adjustment, making it an even better value for buyers!
...
${property.neighborhood ? `Come see why ${property.neighborhood} is such a desirable area!` : 'Come see this beautiful property!'}
...
${property.neighborhood ? `${property.neighborhood} living awaits!` : 'Your dream home awaits!'} 
...
Join me Saturday & Sunday, 1:00 PM - 4:00 PM for an exclusive tour of this stunning ${property.bedrooms} bedroom, ${property.bathrooms} bathroom home!
...
#OpenHouse #WeekendViewing #OmahaRealEstate #MikeBjork #HomeTour`
    ...
    return `Check out this amazing property at ${property.address}! ${formatPrice(property.listPrice)} | Contact Mike Bjork for details.`;
  };
  ...
  const handlePost = () => {
    ...
    if (selectedProperty && !postContent.trim() && selectedPostType) {
      ...
      content = generatePropertyContent(selectedProperty, selectedPostType, primaryPlatform);
    }
    
    if (!content) {
      toast({
        title: "Content Required",
        description: "Please enter content to post or select a property with post type",
        variant: "destructive",
      });
      return;
    }
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-lg font-semibold text-foreground">Social Media Manager</CardTitle>
      </CardHeader>
      <CardContent className="space-y-6">
        {/* Platform Selection */}
        <div className="space-y-3">
          <h3 className="text-sm font-medium text-foreground">Select Platforms</h3>
          {accounts?.map((account) => {
            const platformInfo = platformIcons[account.platform as keyof typeof platformIcons];
                ...
                <div className="flex items-center space-x-3">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => 
                      handlePlatformToggle(account.platform, account.isConnected)
                    }
                    disabled={!account.isConnected}
                    className="h-5 w-5 bg-[#2d4450] text-[#304652]"
                    data-testid={`checkbox-${account.platform}`}
                  />
                  <platformInfo.icon className={`h-4 w-4 ${platformInfo.color}`} />
        ...
        {selectedPlatforms.includes('youtube') && (
          <div className="space-y-3 p-4 bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800 rounded-lg">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-medium text-foreground">YouTube Video Upload</h3>
            </div>
            <p className="text-xs text-muted-foreground">
              Upload a video file to post directly to your YouTube channel as a public video.
            </p>
            <div className="flex items-center gap-2">
              <Button
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <Button
              ...
              disabled={optimizeContentMutation.isPending || !postContent.trim()}
              variant="ghost"
              size="sm"
            ...
            >
              ...
              {optimizeContentMutation.isPending ? "Optimizing..." : "AI Optimize"}
            </Button>
          </div>
          
          {/* Property Selector */}
          <div className="space-y-2">
            <div className="text-xs font-medium text-muted-foreground mb-2">Property Listing (Optional)</div>
            <PropertySelector 
          ...
          {/* Post Type Selection */}
          <div className="space-y-2">
            <div className="text-xs font-medium text-muted-foreground mb-2">Post Type (Optional)</div>
            <div className="grid grid-cols-2 gap-2">
              {postTypes.map((type) => (
                <Button
                  ...
                  onClick={() => {
                    const newType = selectedPostType === type.id ? null : type.id;
                    ...
                    if (selectedProperty && newType && selectedPlatforms.length > 0 && type.id !== "create_your_own") {
                      const primaryPlatform = selectedPlatforms[0];
                      const generatedContent = generatePropertyContent(selectedProperty, newType, primaryPlatform);
                ...
                >
                  <div className="p-1.5 rounded-md bg-[#2d4450]">
                    <type.icon className={`h-3.5 w-3.5 ${selectedPostType === type.id ? type.color : "text-muted-foreground"}`} />
                  </div>
                  {type.label}
                </Button>
              ))}
          ...
          <Textarea
            placeholder={selectedPostType 
              ? `Enter details for ${postTypes.find(t => t.id === selectedPostType)?.label.toLowerCase()} post (address, price, features, etc.)...` 
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-muted-foreground">
              💡 Use this to guide AI optimization with specific instructions or tone preferences
            </p>
          </div>
          
          {selectedPlatforms.length > 0 && (
            <div className="text-xs text-muted-foreground">
              Posting to: {selectedPlatforms.map(p => p.charAt(0).toUpperCase() + p.slice(1)).join(", ")}
            </div>
          )}
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
                ...
                <DialogContent className="max-w-md">
                  ...
                  <div className="space-y-4">
                    <div className="text-sm text-muted-foreground">
                      Posting to: {selectedPlatforms.length > 0 ? selectedPlatforms.map(p => p.charAt(0).toUpperCase() + p.slice(1)).join(", ") : "No platforms selected"}
                    </div>
                    <div className="border rounded-lg p-4 bg-muted/30">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <div className="font-medium text-sm text-foreground">Mike Bjork</div>
                          <div className="text-xs text-muted-foreground mb-2">Bjork Group at BHHS</div>
                          <div className="text-sm text-foreground whitespace-pre-wrap">{postContent}</div>
                          {uploadedPhoto && (
                          ...
                          {selectedProperty && (
                            <div className="mt-3 p-3 bg-background rounded-md border">
                              ...
                              <div className="text-xs text-muted-foreground">{selectedProperty.city}, {selectedProperty.state}</div>
                              <div className="text-sm font-medium mt-1">${selectedProperty.listPrice?.toLocaleString() || '0'}</div>
              ...
              <Button 
                variant="ghost" 
                size="icon" 
                className={`relative ${uploadedPhoto ? "text-green-600 hover:text-green-700" : "text-muted-foreground hover:text-foreground"}`}
              ...
              <Button variant="ghost" size="icon" className="text-muted-foreground hover:text-foreground" data-testid="button-schedule">
        ...
        {/* Upcoming Posts */}
        <div>
          <h3 className="text-sm font-medium text-foreground mb-3">Scheduled Posts</h3>
          <div className="space-y-2">
            {scheduledPosts.map((post) => (
              <div key={post.id} className="p-3 bg-muted rounded-md" data-testid={`scheduled-post-${post.id}`}>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Upload a photo to make your social media post more engaging. This photo will be included with your post content.
            </p>
                  ...
                  <ObjectUploader
                    ...
                    onComplete={(uploadedFileUrl: string) => {
                      ...
                      toast({
                        ...
                        description: "Your photo is ready to be included in your post",
                      });
                    }}
                ...
                <TabsContent value="ai" className="space-y-4">
                  <div className="space-y-3">
                    ...
                    <div className="text-xs text-muted-foreground text-center">
                      AI will create a custom image based on your description, perfectly tailored for your social media post.
                    </div>
                  </div>
                </TabsContent>
              </Tabs>
            ...
            <div className="flex items-center gap-2 pt-2">
              <Button 
                ...
                onClick={() => {
                  ...
                  if (selectedPostType === "create_your_own" && !uploadedPhoto) {
                    setSelectedPostType(null);
                  }
                }}
              >
      ...
      <Dialog open={showVideoUpload} onOpenChange={setShowVideoUpload}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Upload a video to post to your YouTube channel. This will create a public video on your channel.
            </p>
            ...
            {uploadedVideo ? (
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  <p className="text-sm text-green-600 font-medium">Video ready for upload!</p>
                  <Button 
                    variant="outline" 
                    size="sm"
