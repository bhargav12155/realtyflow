...
- **HeyGen Avatar Generation Flow (External Service Proxy)**: The "Generate AI Enhanced Look" feature on the Photo Avatars page allows users to upload a photo or select an existing avatar, then generate 4 professional AI-enhanced looks via an external service (AWS Elastic Beanstalk). The flow uses proxy endpoints: `POST /api/photo-avatars/create-with-looks` (upload image, returns immediately with group_id, ~6-8 min background processing), `GET /api/photo-avatars/status/:groupId` (poll for completion), `POST /api/photo-avatars/generate-video-from-image` (upload image + script, returns immediately, ~8-13 min background processing), `GET /api/photo-avatars/video-status/:videoId` (poll for video completion). All proxied to `PHOTO_AVATAR_SERVICE_URL` env var or default Elastic Beanstalk URL.
