...
**WhatsApp Bulk Queue System**: When bulk sends exceed Meta's daily messaging limit, overflow contacts are auto-queued in `whatsapp_bulk_queues` table. Background scheduler (`server/services/bulk-queue-scheduler.ts`) checks every 60 seconds, processes active queues when `nextBatchAt` is reached (24-hour rolling window), sends in staggered batches of 8 (150ms spacing per message to stay under Meta's 10 req/sec API limit) with retry logic for error codes 130429 (throughput), 429 (rate), 503 (overload), and graceful skip for 131056 (pair rate limit) and 131049 (user frequency cap). API endpoints: `GET /api/whatsapp/bulk-queues` (list user's queues), `GET/POST /api/whatsapp/bulk-queues/:id` (get/pause/resume/cancel with ownership checks). Frontend shows queue status dashboard with progress bars, sent/remaining/quota/next-batch stats, and pause/resume/cancel buttons. WebSocket events: `whatsapp_queue_progress`, `whatsapp_queue_batch_start`, `whatsapp_queue_batch_complete`, `whatsapp_queue_complete`. **Meta tier mapping (post-Oct 2025, portfolio-level)**: Uses `whatsapp_business_manager_messaging_limit` field (deprecated `messaging_limit_tier`). TIER_NOT_SET=250, TIER_250=250, TIER_2K=2,000, TIER_10K=10,000, TIER_100K=100,000, TIER_UNLIMITED=999,999. Limits are portfolio-level, shared across all phone numbers. Graph API v25.0.
