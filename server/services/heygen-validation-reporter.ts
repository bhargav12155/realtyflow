 ...
 *      critical-severity admin alert. The burst alert is what feeds into
 *      the on-call/Slack channel via the team's log-based alerting — see
...
export function registerHeygenValidationReporter(): void {
  ...
  setHeygenValidationReporter((report: HeygenValidationFailureReport) => {
    ...
    // structured log line + a critical admin alert. Log-based alerting
    // (e.g. Slack/PagerDuty rule on `heygen.response.invalid.burst`) is
