...
export class SocialMediaService {
  async postToFacebook(content: string, accessToken?: string, imageUrl?: string): Promise<{ postId: string }> {
    ...
    throw new Error('Direct posting to personal Facebook profiles is not supported. Please use page posting instead.');
  }

  async postToYoutube(title: string, description: string, videoUrl?: string, accessToken?: string): Promise<{ postId: string }> {
    try {
      if (!accessToken) {
        throw new Error('YouTube access token required for posting');
      }

      console.log('YouTube postToYoutube called with accessToken:', accessToken);
      ...
      if (videoUrl) {
        ...
        return await this.uploadVideoToYoutube(title, description, videoUrl, accessToken);
      } else {
        // Create a Community post (text-only)
        return await this.createYoutubeCommunityPost(title, description, accessToken);
      }
    } catch (error) {
  ...
  private async uploadVideoToYoutube(title: string, description: string, videoUrl: string, accessToken: string): Promise<{ postId: string }> {
    try {
      console.log('Starting YouTube video upload:', { title, description, videoUrl });

      // Check if this is a mock token for testing
      if (accessToken === 'mock_youtube_token' || accessToken === 'mock_token') {
        console.log('Mock YouTube video upload simulated:', { title, description, videoUrl });
        return {
      ...
      if (errorMessage.includes('forbidden') || errorMessage.includes('403')) {
        throw new Error('YouTube upload permission denied. Check your channel permissions.');
      }
  ...
  private async createYoutubeCommunityPost(title: string, description: string, accessToken: string): Promise<{ postId: string }> {
    try {
      // Check if this is a mock token
      if (accessToken === 'mock_youtube_token' || accessToken === 'mock_token') {
        console.log('Mock YouTube Community Post created:', { title, description });
        return {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          throw new Error(`YouTube API authentication test failed: ${response.status}`);
        }
      ...
      console.log('YouTube authentication successful! Channel:', channelData.items?.[0]?.snippet?.title);
  ...
  async postToInstagram(content: string, imageUrl?: string, accessToken?: string, igUserId?: string): Promise<{ postId: string }> {
    try {
      ...
      const containerResponse = await fetch(`https://graph.facebook.com/v21.0/${userId}/media`, {
        method: 'POST',
      ...
      if (!containerResponse.ok) {
        ...
        if (errorData.error?.code === 190) {
          throw new Error('Instagram setup incomplete. Token must be from Facebook page admin with connected Instagram business account.');
        }
        if (errorData.error?.code === 100) {
          throw new Error('Invalid Instagram parameters. Please check your content and image.');
        }
      ...
      const publishResponse = await fetch(`https://graph.facebook.com/v21.0/${userId}/media_publish`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      ...
      if (!publishResponse.ok) {
        ...
        if (errorData.error?.code === 100) {
          throw new Error('Failed to publish Instagram content. Please try again.');
        }
        
        throw new Error(`Instagram publishing failed: ${errorData.error?.message || 'Unknown error'}`);
      }
  ...
  async postToLinkedIn(content: string, accessToken: string): Promise<{ postId: string }> {
    try {
  ...
  async postToTwitter(content: string, imageUrl?: string): Promise<{ postId: string }> {
    try {
      ...
      if (!response.ok) {
        ...
        if (errorData.errors?.[0]?.code === 403) {
          throw new Error('Twitter API access denied. Please check your credentials and permissions.');
        }
        if (errorData.errors?.[0]?.code === 401) {
          throw new Error('Twitter authentication failed. Please verify your API keys.');
        }
        
        throw new Error(`Twitter posting failed: ${errorData.detail || errorData.errors?.[0]?.message || 'Unknown error'}`);
      }
  ...
  async deleteTwitterPost(tweetId: string): Promise<{ success: boolean }> {
    try {
      ...
      if (!response.ok) {
        ...
        if (errorData.errors?.[0]?.code === 403) {
          throw new Error('Twitter delete access denied. Check your credentials and permissions.');
        }
        if (errorData.errors?.[0]?.code === 401) {
          throw new Error('Twitter authentication failed for delete operation.');
        }
        if (errorData.errors?.[0]?.code === 404) {
          throw new Error('Tweet not found or already deleted.');
        }
        
        throw new Error(`Twitter delete failed: ${errorData.detail || errorData.errors?.[0]?.message || 'Unknown error'}`);
      }
  ...
  async schedulePost(post: SocialMediaPost, platforms: string[]): Promise<{ scheduledId: string }> {
    try {
      // Implementation would integrate with a job scheduler (like node-cron or a queue system)
      console.log('Scheduling post for platforms:', platforms, 'at:', post.scheduledFor);
  ...
  async getMetrics(platform: string, accessToken: string): Promise<SocialMediaMetrics> {
    try {
  ...
  async validateConnection(platform: string, accessToken?: string): Promise<boolean> {
    try {
      ...
      if (platform === 'instagram') {
        ...
        const response = await fetch(`https://graph.facebook.com/v21.0/${userId}?fields=id,username&access_token=${token}`);
        return response.ok;
      }
      ...
      if (platform === 'twitter') {
        ...
        if (!consumerKey || !consumerSecret || !accessToken || !accessTokenSecret) return false;
  ...
  async getFacebookPageInfo(accessToken?: string): Promise<{ id: string; name: string; category: string }[]> {
    try {
      const token = accessToken || process.env.FACEBOOK_USER_TOKEN;
      if (!token) {
  ...
  async postToFacebookPage(pageId: string, content: string, imageUrl?: string, accessToken?: string, baseUrl?: string): Promise<{ postId: string }> {
    try {
      const token = accessToken || process.env.FACEBOOK_USER_TOKEN;
      if (!token) {
        throw new Error('Facebook access token not available');
      ...
      if (!pagesResponse.ok) {
        ...
        if (errorData.error?.code === 200) {
          throw new Error('Insufficient permissions. Please grant pages access to your Facebook account.');
        }
        throw new Error(`Failed to fetch page access token: ${errorData.error?.message || 'Unknown error'}`);
      }
      ...
      if (!page) {
        throw new Error('Page not found or no access. Please check page permissions.');
      }
      ...
      if (!response.ok) {
        ...
        if (errorData.error?.code === 200) {
          throw new Error('Insufficient permissions for this page. Please check page roles.');
        }
        if (errorData.error?.code === 100) {
          throw new Error('Invalid parameters. Please check your content and try again.');
        }
        
        throw new Error(`Facebook posting failed: ${errorData.error?.message || 'Unknown error'}`);
      }
